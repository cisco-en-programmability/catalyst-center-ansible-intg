  ...
  config:
    ...
    suboptions:
      ...
      generate_yaml_config:
        description:
          - When set to True generates YAML configurations compatible with the `wireless_design_workflow_manager` module, enabling programmatic
            modifications and reducing the effort required to manually create Ansible playbooks.
        type: bool
      yaml_config_options:
        description:
          - Options for generating the YAML configuration file.
        type: dict
        suboptions:
          file_path:
            description:
              - Path where the YAML configuration file will be saved.
              - If the file path is not provided, a default file name is "generated module_name_playbook_DD_Mon_YYYY_HH_MM_SS_MS.yml" and the
          ...
          component_specific_filters:
            description:
              ...
              - If "components_list" is specified, only those components are included, even if other filters are provided.
            type: dict
            suboptions:
              components_list:
                description:
                  - List of components to include in the YAML configuration file.
                  - Valid values are ["ssids", "anchor_groups", "power_profiles", "ap_profiles", "rf_profiles", "interfaces"].
                type: list
                elements: str
              site_name_hierarchy:
                description:
...
class WirelessDesign(DnacBase):
    ...
    def validate_input(self):
        ...
        self.temp_spec = {
            ...
            "generate_yaml_config": {"type": "bool", "required": False, "default": False},
            "yaml_config_options": {
                "type": "dict",
    ...
    def wireless_design_workflow_manager_mapping(self):
        ...
        return {
            "network_elements": {
                "ssids": {
                    "filters": [
                        "site_name_hierarchy", "ssid_names_list"
                    ],
    ...
    def validate_global_filters(self, global_filters):
        ...
        self.log("Starting validation of global filters for module: {0}".format(self.module_name), "INFO")
        ...
        if not valid_global_filters and global_filters:
            self.msg = (
                "Module '{0}' does not support global filters, but 'global_filters' were provided: {1}. Please remove them."
                .format(self.module_name, list(global_filters.keys()))
            )
        ...
        invalid_filters = [key for key in global_filters.keys() if key not in valid_global_filters]

        # If any invalid filters are found, log the error and exit the operation
        if invalid_filters:
            self.msg = (
                "Invalid 'global_filters' found for module '{0}': {1}. Valid 'global_filters' are: {2}"
                .format(self.module_name, invalid_filters, valid_global_filters)
            )
            self.log(self.msg, "ERROR")
            self.fail_and_exit(self.msg)
    ...
    def validate_yaml_config_generator_params(self, yaml_config_generator, state):
        """
        Validates the parameters provided for the YAML configuration generator.
        Args:
            yaml_config_generator (dict): A dictionary containing the configuration parameters
        ...
        """
        self.log("Starting validation of 'yaml_config_generator' parameters in state: {0}.".format(state), "INFO")
        # self.module_mapping = self.wireless_design_workflow_manager_mapping()
        ...
        if global_filters:
            self.log("Validating 'global_filters' for module '{0}': {1}.".format(self.module_name, global_filters), "INFO")
        ...
        if component_specific_filters:
            self.log("Validating 'component_specific_filters' for module '{0}': {1}.".format(self.module_name, component_specific_filters), "INFO")
            self.validate_component_specific_filters(component_specific_filters)
        else:
            self.log("No 'component_specific_filters' provided for module '{0}'; skipping validation.".format(self.module_name), "INFO")

        self.log("Completed validation of 'yaml_config_generator' parameters module: {0}".format(self.module_name), "INFO")
    ...
    def validate_params(self, config, state):
        ...
        validation_mapping = {
            ...
            "yaml_config_options": self.validate_yaml_config_generator_params,
        }
    ...
    def generate_filename(self):
        ...
        self.log("Filename generation process completed successfully: {0}".format(filename), "INFO")
        return filename

    def ensure_directory_exists(self, file_path):
        """Ensure the directory for the file path exists."""
        self.log("Starting 'ensure_directory_exists' for file path: {0}".format(file_path), "INFO")
        ...
        if directory and not os.path.exists(directory):
            self.log("Directory '{0}' does not exist. Creating it.".format(directory), "INFO")
            os.makedirs(directory)
            self.log("Directory '{0}' created successfully.".format(directory), "INFO")
        else:
            self.log("Directory '{0}' already exists. No action needed.".format(directory), "INFO")
    ...
    def get_global_site_details(self):
        ...
        Returns:
            str: Global site ID.
        """
        self.log("Starting retrieval of global site details.", "INFO")
        global_site_name = "Global"
        self.log("Fetching details for global site: {0}".format(global_site_name), "INFO")

        # Directly retrieve the global site ID
        self.log("Attempting to retrieve site ID for global site: {0}".format(global_site_name), "INFO")
        global_site_id = self.get_site_id(global_site_name)[1]
        self.log("Successfully retrieved global site ID: {0}".format(global_site_id), "DEBUG")

        self.log("Completed retrieval of global site details.", "INFO")
    ...
    def wireless_ssid_temp_spec(self):
        ...
        wireless_ssid_temp_spec = OrderedDict({
            ...
            "radio_policy": {"type": "dict", "options": OrderedDict({
                "radio_bands": {"type": "list", "source_key": "ssidRadioType", "transform": lambda x: {
                ...
                "2_dot_4_ghz_band_policy": {"type": "str", "source_key": "ghz24Policy", "transform": lambda x: {
            ...
            "l2_security": {"type": "dict", "options": OrderedDict({
                ...
                "mpsk_settings": {"type": "list", "elements": "dict", "source_key": "multiPSKSettings", "options": OrderedDict({
                    "mpsk_priority": {"type": "int", "source_key": "priority"},
                    "mpsk_passphrase_type": {"type": "str", "source_key": "passphraseType"},
                    "mpsk_passphrase": {
                        "type": "str",
                        "special_handling": True,
                        "transform": lambda ssid_details: self.generate_custom_variable_name(
                            ssid_details, "mpsk_settings", "priority", "mpsk_passphrase"
                        )
                    },
                })},
            })},
            ...
            "l3_security": {"type": "dict", "options": OrderedDict({
                "l3_auth_type": {"type": "str", "source_key": "l3AuthType", "transform": lambda x: {
                    "WEB_AUTH": "web_auth",
                    "OPEN": "open"
                }.get(x, x)},
                "auth_server": {"type": "str", "source_key": "authServer", "transform": lambda x: {
            ...
            "aaa": {"type": "dict", "options": OrderedDict({
            ...
            "wlan_timeouts": {"type": "dict", "options": OrderedDict({
            ...
            "bss_transition_support": {"type": "dict", "options": OrderedDict({
            ...
            "sites_specific_override_settings": {
                ...
                "elements": "dict",
                "special_handling": True,
                "transform": lambda ssid_details: self.modify_parameters(
                    OrderedDict({
                        "site_name_hierarchy": {"type": "str", "source_key": "site_name_hierarchy"},
                        "wlan_profile_name": {"type": "str", "source_key": "profileName"},
                        "l2_security": {"type": "dict", "options": OrderedDict({
                            "l2_auth_type": {"type": "str", "source_key": "authType"},
                            "open_ssid": {"type": "str", "source_key": "openSsid"},
                            "passphrase": {
                                ...
                                "transform": lambda site_override: self.generate_custom_variable_name(
                                    site_override, "ssid", "ssid", "passphrase"
                                )
                            },
                            "mpsk_settings": {"type": "list", "elements": "dict", "source_key": "multiPSKSettings", "options": OrderedDict({
                                "mpsk_priority": {"type": "int", "source_key": "priority"},
                                ...
                                "mpsk_passphrase": {
                                    ...
                                    "special_handling": True,
                                    "transform": lambda site_override: self.generate_custom_variable_name(
                                        site_override, "mpsk_settings", "priority", "mpsk_passphrase"
                                    )
                                },
                            })}
                        })},
                        ...
                        "wpa_encryption": {
                            ...
                            "transform": lambda site_override: self.process_boolean_to_list(
                                site_override if isinstance(site_override, dict) else {},
                                {
                                    "GCMP256": "rsnCipherSuiteGcmp256",
                                    "CCMP256": "rsnCipherSuiteCcmp256",
                                    "GCMP128": "rsnCipherSuiteGcmp128",
                ...
                ) if "sites_specific_override_settings" in ssid_details else []
            }
        })
        return wireless_ssid_temp_spec

    def combine_global_and_site_ssids(self, global_ssids, site_ssids, site_name_hierarchy):
        """
        ...
        """
        self.log("Starting combination of global and site-specific SSIDs.", "INFO")
        self.log("Global SSIDs: {0}".format(global_ssids), "DEBUG")
        self.log("Site SSIDs: {0}".format(site_ssids), "DEBUG")
        self.log("Site name hierarchy: {0}".format(site_name_hierarchy), "DEBUG")

        # Create a dictionary of global SSIDs for quick lookup by ssid_name
        combined_ssids = {ssid["ssid"]: ssid for ssid in global_ssids}
        self.log("Initialized combined SSIDs dictionary with global SSIDs.", "DEBUG")

        for site_ssid in site_ssids:
            ssid_name = site_ssid["ssid"]
            self.log("Processing site-specific SSID: {0}".format(site_ssid), "DEBUG")

            # Add the site_name_hierarchy to the site-specific SSID
            site_ssid["site_name_hierarchy"] = site_name_hierarchy
            self.log("Added site_name_hierarchy '{0}' to site-specific SSID.".format(site_name_hierarchy), "DEBUG")

            if ssid_name in combined_ssids:
                self.log("SSID '{0}' exists in global SSIDs. Applying site-specific overrides.".format(ssid_name), "INFO")
                if "sites_specific_override_settings" not in combined_ssids[ssid_name]:
                    ...
                    self.log("Initialized 'sites_specific_override_settings' for SSID '{0}'.".format(ssid_name), "DEBUG")
                combined_ssids[ssid_name]["sites_specific_override_settings"].append(site_ssid)
                self.log("Appended site-specific SSID to 'sites_specific_override_settings' for SSID '{0}'.".format(ssid_name), "DEBUG")
            else:
                self.log("SSID '{0}' does not exist in global SSIDs. Adding as a new entry.".format(ssid_name), "INFO")
                combined_ssids[ssid_name] = site_ssid

        combined_ssids_list = list(combined_ssids.values())
        self.log("Completed combination of SSIDs. Combined SSIDs: {0}".format(combined_ssids_list), "INFO")
        return combined_ssids_list
    ...
    def get_wireless_ssids(self, network_element, component_specific_filters=None):
        ...
        self.log("Getting wireless SSIDs using family '{0}' and function '{1}'.".format(api_family, api_function), "INFO")
        ...
        if component_specific_filters:
            ...
            if site_name_hierarchy:
                ...
                if not site_exists:
                    ...
                    self.fail_and_exit(self.msg)
                self.log("Site '{0}' found with ID: {1}".format(site_name_hierarchy, site_id), "DEBUG")
                ...
                if not ssid_names_list:
                    ...
                    self.log("Created params for specified site: {0}".format(site_params), "DEBUG")
                    site_ssids = self.execute_get_with_pagination(api_family, api_function, site_params)

                    # Combine global and site-specific SSIDs
                    combined_ssids = self.combine_global_and_site_ssids(global_ssids, site_ssids, site_name_hierarchy)
                ...
                # If both site_name_hierarchy and ssid_names_list are provided
                    ...
                    for ssid_name in ssid_names_list:
                        ...
                        global_params = self.get_ssids_params(site_id=global_site_id, ssid_name=ssid_name)
                        self.log("Created params for global site with SSID '{0}': {1}".format(ssid_name, global_params), "DEBUG")
                        global_ssid = self.execute_get_with_pagination(api_family, api_function, global_params)

                        # Create params for the specified site with SSID name
                        site_params = self.get_ssids_params(site_id=site_id, ssid_name=ssid_name)
                        self.log("Created params for specified site with SSID '{0}': {1}".format(ssid_name, site_params), "DEBUG")
                        site_ssid = self.execute_get_with_pagination(api_family, api_function, site_params)

                        # Combine global and site-specific SSIDs for this SSID name
                        combined_ssids = self.combine_global_and_site_ssids(global_ssid, site_ssid, site_name_hierarchy)
                        accumulated_results.extend(combined_ssids)
            else:
                # If no site is specified, use the global site ID
                for ssid_name in ssid_names_list:
                    params = self.get_ssids_params(site_id=global_site_id, ssid_name=ssid_name)
                    self.log("Created params for global site: {0}".format(params), "DEBUG")
                    accumulated_results.extend(self.execute_get_with_pagination(api_family, api_function, params))
        else:
            # If no filters are provided, use the global site ID
            self.log("No filters provided. Using global site details for API call.", "INFO")
            global_params = self.get_ssids_params(site_id=global_site_id)
            self.log("Created params for global site: {0}".format(global_params), "DEBUG")
            accumulated_results.extend(self.execute_get_with_pagination(api_family, api_function, global_params))

        # Modify SSID details using temp_spec
        wireless_ssid_temp_spec = self.wireless_ssid_temp_spec()
        modified_ssid_details = self.modify_parameters(wireless_ssid_temp_spec, accumulated_results)
    ...
    def get_wireless_interfaces(self, network_element, component_specific_filters=None):
        """
        Retrieves wireless interface details and modifies them according to the specified parameters mapping.
        Args:
            network_element (dict): Contains api_family and api_function for retrieving interface details.
            component_specific_filters (dict, optional): Filters to apply when retrieving interface details.
        Returns:
            dict: A dictionary containing modified interface details.
        """
        # Extract API family and function from network_element
        api_family = network_element.get("api_family")
        api_function = network_element.get("api_function")
        self.log("Getting wireless Interfaces using family '{0}' and function '{1}'.".format(api_family, api_function), "INFO")

        # Initialize a list to accumulate interface details
        accumulated_interfaces = []

        # Determine parameters based on component_specific_filters
        if component_specific_filters:
            self.log("Using component-specific filters for API call.", "INFO")
            interface_names_list = component_specific_filters.get("interface_names_list", [])

            # If interface names are provided, retrieve details for each interface
            if interface_names_list:
                self.log("Found interface_names_list: {0}".format(interface_names_list), "DEBUG")
                for interface_name in interface_names_list:
                    self.log("Retrieving details for interface: {0}".format(interface_name), "DEBUG")
                    params = self.get_interfaces_params(interface_name=interface_name)
                    self.log("Created params for interface '{0}': {1}".format(interface_name, params), "DEBUG")
                    interface_details = self.execute_get_with_pagination(api_family, api_function, params)
                    self.log("Retrieved details for interface '{0}': {1}".format(interface_name, interface_details), "DEBUG")
                    accumulated_interfaces.extend(interface_details)
            else:
                self.log("No interface_names_list provided. Using default parameters.", "WARNING")
                params = {}
                accumulated_interfaces.extend(self.execute_get_with_pagination(api_family, api_function, params))
        else:
            self.log("No component-specific filters provided. Using global site details for API call.", "INFO")
            params = {}
            accumulated_interfaces.extend(self.execute_get_with_pagination(api_family, api_function, params))

        # Modify interface details using temp_spec
    ...
    def get_wireless_power_profiles(self, network_element, component_specific_filters=None):
        """
        Retrieves wireless power profile details and modifies them according to the specified parameters mapping.
        Args:
            network_element (dict): Contains api_family and api_function for retrieving power profile details.
            component_specific_filters (dict, optional): Filters to apply when retrieving power profile details.
        Returns:
            dict: A dictionary containing modified power profile details.
        """
        # Extract API family and function from network_element
        api_family = network_element.get("api_family")
        api_function = network_element.get("api_function")
        self.log("Getting wireless Power Profiles using family '{0}' and function '{1}'.".format(api_family, api_function), "INFO")

        # Initialize a list to accumulate power profile details
        accumulated_power_profiles = []

        # Determine parameters based on component_specific_filters
        if component_specific_filters:
            self.log("Using component-specific filters for API call.", "INFO")
            power_profile_names_list = component_specific_filters.get("power_profile_names_list", [])

            # If power profile names are provided, retrieve details for each profile
            if power_profile_names_list:
                self.log("Found power_profile_names_list: {0}".format(power_profile_names_list), "DEBUG")
                for power_profile_name in power_profile_names_list:
                    self.log("Retrieving details for power profile: {0}".format(power_profile_name), "DEBUG")
                    params = self.get_power_profiles_params(power_profile_name=power_profile_name)
                    self.log("Created params for power profile '{0}': {1}".format(power_profile_name, params), "DEBUG")
                    power_profile_details = self.execute_get_with_pagination(api_family, api_function, params)
                    self.log("Retrieved details for power profile '{0}': {1}".format(power_profile_name, power_profile_details), "DEBUG")
                    accumulated_power_profiles.extend(power_profile_details)
            else:
                self.log("No power_profile_names_list provided. Using default parameters.", "WARNING")
                params = {}
                accumulated_power_profiles.extend(self.execute_get_with_pagination(api_family, api_function, params))
        else:
            self.log("No component-specific filters provided. Using global site details for API call.", "INFO")
            params = {}
            accumulated_power_profiles.extend(self.execute_get_with_pagination(api_family, api_function, params))

        # Modify power profile details using temp_spec
        wireless_power_profiles_temp_spec = self.wireless_power_profiles_temp_spec()
        modified_power_profiles_details = self.modify_parameters(wireless_power_profiles_temp_spec, accumulated_power_profiles)
        self.log("Modified Power Profiles details: {0}".format(modified_power_profiles_details), "INFO")

        power_profiles = {"power_profiles": modified_power_profiles_details}
    ...
    def wireless_access_point_profiles_temp_spec(self):
        ...
        country_code_map = {
            ...
            "BE": "Belgium", "BT": "Bhutan", "BO": "Bolivia", "BA": "Bosnia", "BW": "Botswana", "BR": "Brazil",
            "BN": "Brunei", "BG": "Bulgaria", "BI": "Burundi", "KH": "Cambodia", "CM": "Cameroon", "CA": "Canada",
            ...
            "DE": "Germany", "GH": "Ghana", "GI": "Gibraltar", "GR": "Greece", "GT": "Guatemala", "HN": "Honduras",
            "HK": "Hong Kong", "HU": "Hungary", "IS": "Iceland", "IN": "India", "ID": "Indonesia", "IQ": "Iraq",
            "IE": "Ireland", "IM": "Isle of Man", "IL": "Israel", "IT": "Italy", "CI": "Ivory Coast (Cote dIvoire)",
            "JM": "Jamaica", "J2": "Japan 2(P)", "J4": "Japan 4(Q)", "JE": "Jersey", "JO": "Jordan", "KZ": "Kazakhstan",
            "KE": "Kenya", "KR": "Korea Extended (CK)", "XK": "Kosovo", "KW": "Kuwait", "LA": "Laos", "LV": "Latvia",
            "LB": "Lebanon", "LY": "Libya", "LI": "Liechtenstein", "LT": "Lithuania", "LU": "Luxembourg", "MO": "Macao",
            "MK": "Macedonia", "MY": "Malaysia", "MT": "Malta", "MU": "Mauritius", "MX": "Mexico", "MD": "Moldova",
            "MC": "Monaco", "MN": "Mongolia", "ME": "Montenegro", "MA": "Morocco", "MM": "Myanmar", "NA": "Namibia",
            ...
            "RU": "Russian Federation", "SM": "San Marino", "SA": "Saudi Arabia", "RS": "Serbia", "SG": "Singapore",
            "SK": "Slovak Republic", "SI": "Slovenia", "ZA": "South Africa", "ES": "Spain", "LK": "Sri Lanka",
            "SD": "Sudan", "SE": "Sweden", "CH": "Switzerland", "TW": "Taiwan", "TH": "Thailand", "TT": "Trinidad",
            "TN": "Tunisia", "TR": "Turkey", "UG": "Uganda", "UA": "Ukraine", "AE": "United Arab Emirates",
            "GB": "United Kingdom", "TZ": "United Republic of Tanzania", "US": "United States", "UY": "Uruguay",
            "UZ": "Uzbekistan", "VA": "Vatican City State", "VE": "Venezuela", "VN": "Vietnam", "YE": "Yemen",
        ...
        ap_profile_temp_spec = OrderedDict({
            ...
            "management_settings": {
                "type": "dict",
                "options": OrderedDict({
                    "access_point_authentication": {"type": "str", "source_key": "managementSetting.authType"},
                    "dot1x_username": {"type": "str", "source_key": "managementSetting.dot1xUsername"},
                    "dot1x_password": {
                        "type": "str",
                        "special_handling": True,
                        "transform": lambda ssid_details: self.generate_custom_variable_name(
                            ssid_details, "ap_profile", "apProfileName", "dot1x_password"
                        )
                    },
                    "ssh_enabled": {"type": "bool", "source_key": "managementSetting.sshEnabled"},
                    "telnet_enabled": {"type": "bool", "source_key": "managementSetting.telnetEnabled"},
                    "management_username": {"type": "str", "source_key": "managementSetting.managementUserName"},
                    "management_password": {
                        "type": "str",
                        "special_handling": True,
                        "transform": lambda ssid_details: self.generate_custom_variable_name(
                            ssid_details, "ap_profile", "apProfileName", "management_password"
                        )
                    },
                    "management_enable_password": {
                        "type": "str",
                        "special_handling": True,
                        "transform": lambda ssid_details: self.generate_custom_variable_name(
                            ssid_details, "ap_profile", "apProfileName", "management_enable_password"
                        )
                    },
                    "cdp_state": {"type": "bool", "source_key": "managementSetting.cdpState"}
                })
            },
            "security_settings": {
                "type": "dict",
            ...
            "mesh_settings": {
                ...
                "options": OrderedDict({
                    ...
                    "backhaul_client_access": {"type": "bool", "source_key": "meshSetting.backhaulClientAccess"},
                    "rap_downlink_backhaul": {"type": "str", "source_key": "meshSetting.rapDownlinkBackhaul"},
                    "ghz_5_backhaul_data_rates": {"type": "str", "source_key": "meshSetting.ghz5BackhaulDataRates"},
                    "ghz_2_4_backhaul_data_rates": {"type": "str", "source_key": "meshSetting.ghz24BackhaulDataRates"},
                    "bridge_group_name": {"type": "str", "source_key": "meshSetting.bridgeGroupName"}
                })
            },
            "power_settings": {
                "type": "dict",
    ...
    def get_wireless_access_point_profiles(self, network_element, component_specific_filters=None):
        """
        Retrieves wireless access point profile details and modifies them according to the specified parameters mapping.
        Args:
            network_element (dict): Contains api_family and api_function for retrieving access point profile details.
            component_specific_filters (dict, optional): Filters to apply when retrieving access point profile details.
        Returns:
            dict: A dictionary containing modified access point profile details.
        """
        # Extract API family and function from network_element
        api_family = network_element.get("api_family")
        api_function = network_element.get("api_function")
        self.log("Getting wireless Access Point Profiles using family '{0}' and function '{1}'.".format(api_family, api_function), "INFO")

        # Initialize a list to accumulate access point profile details
        accumulated_ap_profiles = []

        # Determine parameters based on component_specific_filters
        if component_specific_filters:
            self.log("Using component-specific filters for API call.", "INFO")
            access_point_profile_names_list = component_specific_filters.get("access_point_profile_names_list", [])

            # If access point profile names are provided, retrieve details for each profile
            if access_point_profile_names_list:
                self.log("Found access_point_profile_names_list: {0}".format(access_point_profile_names_list), "DEBUG")
                for ap_profile_name in access_point_profile_names_list:
                    self.log("Retrieving details for access point profile: {0}".format(ap_profile_name), "DEBUG")
                    params = self.get_access_point_profiles_params(access_point_profile_name=ap_profile_name)
                    self.log("Created params for access point profile '{0}': {1}".format(ap_profile_name, params), "DEBUG")
                    ap_profile_details = self.execute_get_with_pagination(api_family, api_function, params)
                    self.log("Retrieved details for access point profile '{0}': {1}".format(ap_profile_name, ap_profile_details), "DEBUG")
                    accumulated_ap_profiles.extend(ap_profile_details)
            else:
                self.log("No access_point_profile_names_list provided. Using default parameters.", "WARNING")
                params = {}
                accumulated_ap_profiles.extend(self.execute_get_with_pagination(api_family, api_function, params))
        else:
            self.log("No component-specific filters provided. Using global site details for API call.", "INFO")
            params = {}
            accumulated_ap_profiles.extend(self.execute_get_with_pagination(api_family, api_function, params))

        # Modify access point profile details using temp_spec
        ap_profiles_temp_spec = self.wireless_access_point_profiles_temp_spec()
        modified_ap_profiles_details = self.modify_parameters(ap_profiles_temp_spec, accumulated_ap_profiles)
        self.log("Modified Access Point Profiles details: {0}".format(modified_ap_profiles_details), "INFO")
    ...
    def wireless_radio_frequency_profiles_temp_spec(self):
        ...
        radio_frequency_profiles_temp_spec = OrderedDict({
            ...
            "radio_bands_2_4ghz_settings": {
                ...
                "options": OrderedDict({
                    "parent_profile": {"type": "str", "source_key": "radioTypeBProperties.parentProfile"},
                    "dca_channels_list": {"type": "list", "source_key": "radioTypeBProperties.radioChannels"},
                    "supported_data_rates_list": {"type": "list", "source_key": "radioTypeBProperties.dataRates"},
                    "mandatory_data_rates_list": {"type": "list", "source_key": "radioTypeBProperties.mandatoryDataRates"},
                    "minimum_power_level": {"type": "int", "source_key": "radioTypeBProperties.minPowerLevel"},
                    "maximum_power_level": {"type": "int", "source_key": "radioTypeBProperties.maxPowerLevel"},
                    "rx_sop_threshold": {"type": "str", "source_key": "radioTypeBProperties.rxSopThreshold"},
                    "custom_rx_sop_threshold": {"type": "int", "source_key": "radioTypeBProperties.customRxSopThreshold"},
                    "tpc_power_threshold": {"type": "int", "source_key": "radioTypeBProperties.powerThresholdV1"},
                    "client_limit": {"type": "int", "source_key": "radioTypeBProperties.maxRadioClients"},
                    "coverage_hole_detection": {
                        "type": "dict",
                        # "source_key": "coverageHoleDetectionProperties",
                        "options": OrderedDict({
                            "minimum_client_level": {"type": "int", "source_key": "radioTypeBProperties.coverageHoleDetectionProperties.chdClientLevel"},
                            "data_rssi_threshold": {"type": "int", "source_key": "radioTypeBProperties.coverageHoleDetectionProperties.chdDataRssiThreshold"},
                            "voice_rssi_threshold": {"type": "int", "source_key": "radioTypeBProperties.coverageHoleDetectionProperties.chdVoiceRssiThreshold"},
                            "exception_level": {"type": "int", "source_key": "radioTypeBProperties.coverageHoleDetectionProperties.chdExceptionLevel"},
                        })
                    },
                    "spatial_reuse": {
                        "type": "dict",
            ...
            "radio_bands_5ghz_settings": {
                ...
                "options": OrderedDict({
                    "parent_profile": {"type": "str", "source_key": "radioTypeAProperties.parentProfile"},
                    "channel_width": {"type": "str", "source_key": "radioTypeAProperties.channelWidth"},
                    "preamble_puncturing": {"type": "bool", "source_key": "radioTypeAProperties.preamblePuncture"},
                    "zero_wait_dfs": {"type": "bool", "source_key": "radioTypeAProperties.zeroWaitDfsEnable"},
                    "dca_channels_list": {"type": "list", "source_key": "radioTypeAProperties.radioChannels"},
                    "supported_data_rates_list": {"type": "list", "source_key": "radioTypeAProperties.dataRates"},
                    "mandatory_data_rates_list": {"type": "list", "source_key": "radioTypeAProperties.mandatoryDataRates"},
                    "minimum_power_level": {"type": "int", "source_key": "radioTypeAProperties.minPowerLevel"},
                    "maximum_power_level": {"type": "int", "source_key": "radioTypeAProperties.maxPowerLevel"},
                    "rx_sop_threshold": {"type": "str", "source_key": "radioTypeAProperties.rxSopThreshold"},
                    "custom_rx_sop_threshold": {"type": "int", "source_key": "radioTypeAProperties.customRxSopThreshold"},
                    "tpc_power_threshold": {"type": "int", "source_key": "radioTypeAProperties.powerThresholdV1"},
                    "client_limit": {"type": "int", "source_key": "radioTypeAProperties.maxRadioClients"},
                    "coverage_hole_detection": {
                        "type": "dict",
                        "source_key": "coverageHoleDetectionProperties",
                        "options": OrderedDict({
                            "minimum_client_level": {"type": "int", "source_key": "radioTypeAProperties.coverageHoleDetectionProperties.chdClientLevel"},
                            "data_rssi_threshold": {"type": "int", "source_key": "radioTypeAProperties.coverageHoleDetectionPropertieschdDataRssiThreshold"},
                            "voice_rssi_threshold": {"type": "int", "source_key": "radioTypeAProperties.coverageHoleDetectionPropertieschdVoiceRssiThreshold"},
                            "exception_level": {"type": "int", "source_key": "radioTypeAProperties.coverageHoleDetectionPropertieschdExceptionLevel"},
                        })
                    },
                    "flexible_radio_assigment": {
                        ...
                        "options": OrderedDict({
                            "client_aware": {"type": "bool", "source_key": "radioTypeAProperties.fraPropertiesA.clientAware"},
                            "client_select": {"type": "int", "source_key": "radioTypeAProperties.fraPropertiesA.clientSelect"},
                            "client_reset": {"type": "int", "source_key": "radioTypeAProperties.fraPropertiesA.clientReset"},
                        })
                    },
                    "spatial_reuse": {
                        "type": "dict",
            ...
            "radio_bands_6ghz_settings": {
                ...
                "options": OrderedDict({
                    "parent_profile": {"type": "str", "source_key": "radioType6GHzProperties.parentProfile"},
                    "minimum_dbs_channel_width": {"type": "int", "source_key": "radioType6GHzProperties.minDbsWidth"},
                    "maximum_dbs_channel_width": {"type": "int", "source_key": "radioType6GHzProperties.maxDbsWidth"},
                    "preamble_puncturing": {"type": "bool", "source_key": "radioType6GHzProperties.preamblePuncture"},
                    "psc_enforcing_enabled": {"type": "bool", "source_key": "radioType6GHzProperties.pscEnforcingEnabled"},
                    "dca_channels_list": {"type": "list", "source_key": "radioType6GHzProperties.radioChannels"},
                    "supported_data_rates_list": {"type": "list", "source_key": "radioType6GHzProperties.dataRates"},
                    "mandatory_data_rates_list": {"type": "list", "source_key": "radioType6GHzProperties.mandatoryDataRates"},
                    "minimum_power_level": {"type": "int", "source_key": "radioType6GHzProperties.minPowerLevel"},
                    "maximum_power_level": {"type": "int", "source_key": "radioType6GHzProperties.maxPowerLevel"},
                    "rx_sop_threshold": {"type": "str", "source_key": "radioType6GHzProperties.rxSopThreshold"},
                    "custom_rx_sop_threshold": {"type": "int", "source_key": "radioType6GHzProperties.customRxSopThreshold"},
                    "tpc_power_threshold": {"type": "int", "source_key": "radioType6GHzProperties.powerThresholdV1"},
                    "client_limit": {"type": "int", "source_key": "radioType6GHzProperties.maxRadioClients"},
                    "coverage_hole_detection": {
                        "type": "dict",
                        # "source_key": "coverageHoleDetectionProperties",
                        "options": OrderedDict({
                    ...
                    "flexible_radio_assigment": {
                        ...
                        "options": OrderedDict({
                            "client_reset_count": {"type": "int", "source_key": "radioType6GHzProperties.fraPropertiesC.clientResetCount"},
                            "client_utilization_thresthold": {"type": "int", "source_key": "radioType6GHzProperties.fraPropertiesC.clientUtilizationThreshold"},
                        })
                    },
                    "discovery_frames_6ghz": {"type": "str", "source_key": "radioType6GHzProperties.discoveryFrames6GHz"},
                    "broadcast_probe_response_interval": {"type": "int", "source_key": "radioType6GHzProperties.broadcastProbeResponseInterval"},
                    "multi_bssid": {
                        "type": "dict",
                        ...
                        "options": OrderedDict({
                            ...
                            "twt_broadcast_support": {"type": "bool", "source_key": "radioType6GHzProperties.multiBssidProperties.twtBroadcastSupport"},
                        })
                    },
                    "spatial_reuse": {
                        "type": "dict",
    ...
    def get_wireless_radio_frequency_profiles(self, network_element, component_specific_filters=None):
        """
        Retrieves wireless radio frequency profile details and modifies them according to the specified parameters mapping.
        Args:
            network_element (dict): Contains api_family and api_function for retrieving radio frequency profile details.
            component_specific_filters (dict, optional): Filters to apply when retrieving radio frequency profile details.
        Returns:
            dict: A dictionary containing modified radio frequency profile details.
        """
        # Extract API family and function from network_element
        api_family = network_element.get("api_family")
        api_function = network_element.get("api_function")
        self.log("Getting wireless Radio Frequency Profiles using family '{0}' and function '{1}'.".format(api_family, api_function), "INFO")

        # Initialize a list to accumulate radio frequency profile details
        accumulated_rf_profiles = []

        # Determine parameters based on component_specific_filters
        if component_specific_filters:
            self.log("Using component-specific filters for API call.", "INFO")
            radio_frequency_profile_names_list = component_specific_filters.get("radio_frequency_profile_names_list", [])

            # If radio frequency profile names are provided, retrieve details for each profile
            if radio_frequency_profile_names_list:
                self.log("Found radio_frequency_profile_names_list: {0}".format(radio_frequency_profile_names_list), "DEBUG")
                for rf_profile_name in radio_frequency_profile_names_list:
                    self.log("Retrieving details for radio frequency profile: {0}".format(rf_profile_name), "DEBUG")
                    params = self.get_radio_frequency_profiles_params(radio_frequency_profile_name=rf_profile_name)
                    self.log("Created params for radio frequency profile '{0}': {1}".format(rf_profile_name, params), "DEBUG")
                    rf_profile_details = self.execute_get_with_pagination(api_family, api_function, params)
                    self.log("Retrieved details for radio frequency profile '{0}': {1}".format(rf_profile_name, rf_profile_details), "DEBUG")
                    accumulated_rf_profiles.extend(rf_profile_details)
            else:
                self.log("No radio_frequency_profile_names_list provided. Using default parameters.", "WARNING")
                params = {}
                accumulated_rf_profiles.extend(self.execute_get_with_pagination(api_family, api_function, params))
        else:
            self.log("No component-specific filters provided. Using global site details for API call.", "INFO")
            params = {}
            accumulated_rf_profiles.extend(self.execute_get_with_pagination(api_family, api_function, params))

        # Modify radio frequency profile details using temp_spec
        rf_profiles_temp_spec = self.wireless_radio_frequency_profiles_temp_spec()
        modified_rf_profiles_details = self.modify_parameters(rf_profiles_temp_spec, accumulated_rf_profiles)
        self.log("Modified Radio Frequency Profiles details: {0}".format(modified_rf_profiles_details), "INFO")
    ...
    def get_wireless_anchor_groups(self, network_element, component_specific_filters=None):
        """
        Retrieves wireless anchor group details and modifies them according to the specified parameters mapping.
        Args:
            network_element (dict): Contains `api_family` and `api_function` for retrieving anchor group details.
            component_specific_filters (dict, optional): Filters to apply when retrieving anchor group details.
        Returns:
            dict: A dictionary containing modified anchor group details.
        """
        # Extract API family and function from network_element
        api_family = network_element.get("api_family")
        api_function = network_element.get("api_function")
        self.log("Getting wireless Anchor Groups using family '{0}' and function '{1}'.".format(api_family, api_function), "INFO")
        ...
        # Execute API call to retrieve Anchor Groups details
        anchor_groups_details = self.execute_get_with_pagination(api_family, api_function, params)
        self.log("Retrieved Anchor Groups details: {0}".format(anchor_groups_details), "INFO")

        # Modify Anchor Groups details using temp_spec
        anchor_groups_temp_spec = self.wireless_anchor_groups_temp_spec()
        modified_anchor_groups_details = self.modify_parameters(anchor_groups_temp_spec, anchor_groups_details)
        self.log("Modified Anchor Groups details: {0}".format(modified_anchor_groups_details), "INFO")
    ...
    def modify_parameters(self, temp_spec, details_list):
        """
        Modifies a list of details based on a given template specification.
        Args:
            temp_spec (dict): A dictionary defining the template specification for mapping the details.
                              Each key in the template specifies the target key, its type, source key,
                              and optional transformation logic.
            details_list (list): A list of dictionaries containing the details to be modified.
        Returns:
            list: A list of dictionaries with the modified details based on the template specification.
        """
        ...
        for index, detail in enumerate(details_list):
            ...
            for key, spec in temp_spec.items():
                ...
                self.log("Using transformation function for key '{0}'.".format(key), "DEBUG")

                if spec['type'] == 'dict':
                    mapped_detail[key] = self.modify_parameters(spec['options'], [detail])[0]
                    self.log("Mapped nested dictionary for key '{0}': {1}".format(key, mapped_detail[key]), "DEBUG")
                elif spec['type'] == 'list':
                    if spec.get('special_handling'):
                        self.log("Special handling detected for key '{0}'.".format(key), "DEBUG")
                        mapped_detail[key] = transform(detail)
                        self.log("Mapped detail for key '{0}' using special handling: {1}".format(key, mapped_detail[key]), "DEBUG")
                    else:
                        if isinstance(value, list):
                            mapped_detail[key] = [self.modify_parameters(spec['options'], [v])[0] if isinstance(v, dict) else transform(v) for v in value]
                        else:
                            mapped_detail[key] = transform(value) if value else []
                    self.log("Mapped list for key '{0}' with transformation: {1}".format(key, mapped_detail[key]), "DEBUG")
                elif spec['type'] == 'str' and spec.get('special_handling'):
                    self.log("Special handling for string type detected for key '{0}'.".format(key), "DEBUG")
                    mapped_detail[key] = transform(detail)
                    self.log("Mapped string for key '{0}' using special handling: {1}".format(key, mapped_detail[key]), "DEBUG")
                else:
                    mapped_detail[key] = transform(value)
                    self.log("Mapped '{0}' to '{1}' with transformed value: {2}".format(source_key, key, mapped_detail[key]), "DEBUG")

            modified_details.append(mapped_detail)
            self.log("Finished processing detail {0}. Mapped detail: {1}".format(index, mapped_detail), "INFO")
    ...
    def write_dict_to_yaml(self, data_dict, file_path):
        """
        Converts a dictionary to YAML format and writes it to a specified file.
        Args:
            data_dict (dict): The dictionary to be converted to YAML format.
            file_path (str): The file path where the YAML content will be written.
        Returns:
            bool: True if the YAML content is successfully written to the file, False otherwise.
        """
        try:
            ...
            # Ensure the directory for the file path exists
            self.log("Ensuring the directory exists for file path: {0}".format(file_path), "INFO")
            self.ensure_directory_exists(file_path)

            self.log("Preparing to write YAML content to file: {0}".format(file_path), "INFO")
            with open(file_path, 'w') as yaml_file:
                yaml_file.write(yaml_content)

            self.log("Successfully written YAML content to file: {0}.".format(file_path), "INFO")
            return True

        except Exception as e:
            self.msg = "An error occurred while writing to file '{0}': {1}".format(file_path, str(e))
            self.fail_and_exit(self.msg)
    ...
    def process_boolean_to_list(self, ssid_details, mapping):
        ...
        """
        self.log("Starting process_boolean_to_list with ssid_details: {0}".format(ssid_details), "DEBUG")
        result = []
        for list_value, boolean_key in mapping.items():
            boolean_value = ssid_details.get(boolean_key, False)
            self.log("Checking key '{0}': {1}".format(boolean_key, boolean_value), "DEBUG")
            if boolean_value:  # Check if the boolean key is True
                result.append(list_value)
        self.log("Resulting list: {0}".format(result), "DEBUG")
        return result

    def generate_custom_variable_name(self, network_component_details, network_component, network_component_name_parameter, parameter):
        """
        ...
        self.log("Generating custom variable name for network component: {0}".format(network_component), "DEBUG")
        self.log("Network component details: {0}".format(network_component_details), "DEBUG")
        self.log("Network component name parameter: {0}".format(network_component_name_parameter), "DEBUG")
        self.log("Parameter: {0}".format(parameter), "DEBUG")
        variable_name = "{{ {0}_{1}_{2} }}".format(network_component, network_component_details[network_component_name_parameter], parameter)
        custom_variable_name = "{" + variable_name + "}"
        self.log("Generated custom variable name: {0}".format(custom_variable_name), "DEBUG")
    ...
    def yaml_config_generator(self, yaml_config_generator):
        ...
        global_filters = yaml_config_generator.get("global_filters", {})
        component_specific_filters = yaml_config_generator.get("component_specific_filters", {})
        self.log("Global filters: {0}, Component-specific filters: {1}".format(global_filters, component_specific_filters), "DEBUG")
        ...
        for component in components_list:
            ...
            filters = {key: component_specific_filters[key] for key in network_element.get("filters", []) if key in component_specific_filters}
            self.log("Filters for {0}: {1}".format(component, filters), "DEBUG")

            operation_func = network_element.get("get_function_name")
            if callable(operation_func):
                details = operation_func(network_element, filters)
                self.log("Details retrieved for {0}: {1}".format(component, details), "DEBUG")
                final_list.append(details)

        if not final_list:
            self.msg = "No configurations or components to process for module '{0}'. Verify input filters or configuration.".format(self.module_name)
            self.set_operation_result("ok", False, self.msg, "INFO")
            return self
        ...
        if self.write_dict_to_yaml(final_dict, file_path):
            self.msg = {"YAML config generation Task succeeded for module '{0}'.".format(self.module_name): {"file_path": file_path}}
            self.set_operation_result("success", True, self.msg, "INFO")
        else:
            self.msg = {"YAML config generation Task failed for module '{0}'.".format(self.module_name): {"file_path": file_path}}
            self.set_operation_result("failed", True, self.msg, "ERROR")

        return self

    def process_yaml_config_params(self, config):
        """
        Processes the YAML configuration options and returns the yaml_config_generator dictionary.
        Args:
            config (dict): The configuration dictionary from the playbook.
        Returns:
            dict: The yaml_config_generator dictionary with file_path and component_specific_filters.
        """
        self.log("Processing YAML configuration options.", "INFO")

        # Initialize yaml_config_generator
        yaml_config_generator = {}

        # Check if yaml_config_options is provided
        yaml_config_options = config.get("yaml_config_options", {})
